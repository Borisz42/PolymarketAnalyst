...
class ConservativeStrategy(Strategy):
    ...
    def decide(self, market_data_point, current_capital):
        ...
        if qty_yes == 0 and qty_no == 0:
            ...
            # Proactively check for a hedge
            hedge_side = "Down" if side == "Up" else "Up"
            hedge_price = market_data_point.get("DownAsk") if hedge_side == "Down" else market_data_point.get("UpAsk")

            if hedge_price is None or hedge_price <= 0 or hedge_price >= 1.0:
                return None # No hedge available
