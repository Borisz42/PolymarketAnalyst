...
-   **`AvgArbitrageStrategy`**: A stateful arbitrage strategy that aims to build a balanced position at a favorable average price. It initiates a position on one side and then waits for an opportunity to buy the opposing side at a price that ensures the total cost of the pair is less than $1.00, adjusted for a configurable margin.

## Analysis Scripts

This module also contains several standalone scripts for deeper data analysis.

-   **`analyze_prices.py`**: A utility script that provides a comprehensive statistical summary of a given market data file. It calculates statistics for price and liquidity (min, max, mean, std) and provides a summary of how many markets in the file resolved "Up" versus "Down".

-   **`signal_accuracy_checker.py`**: This script evaluates the historical accuracy of the signals generated by the `PredictionStrategy` and `MovingAverageStrategy`. It runs the strategies on historical data, compares the generated signals to the actual market outcomes, and provides an accuracy report, breaking down performance by signal type and strength.

-   **`reverse_engineer.py`**: A powerful tool for analyzing a user's past trading history to understand the market conditions that led to each trade. By merging a user's trade log with the corresponding market data, it reconstructs the state of the portfolio *just before* each trade was made. The script generates two output files in the `logs/` directory: `reverse_engineering_summary.txt` and a detailed `reverse_engineering_log.csv`, providing valuable insights into a trader's decision-making process.
