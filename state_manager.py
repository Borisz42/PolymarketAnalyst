...
class StateManager:
    ...
    HISTORY_ACCUMULATION = "gabagool:history:accumulation"
    HISTORY_EXPOSURE = "gabagool:history:exposure"
    HISTORY_ARBITRAGE = "gabagool:history:arbitrage"
    ...
    async def add_history_snapshot(self, key: str, data: dict, max_points: int = 100) -> bool:
        """Add a snapshot to a historical series."""
        try:
            ...
            # Trim to max points
            await self.redis.ltrim(key, 0, max_points - 1)
            
            return True
        except Exception as e:
            logger.error("Error adding history snapshot to %s: %s", key, e)
            return False

    async def get_history(self, key: str) -> List[dict]:
        """Retrieve historical series (newest first)."""
        try:
            data = await self.redis.lrange(key, 0, -1)
            return [json.loads(d) for d in data]
        except Exception as e:
            logger.error("Error retrieving history from %s: %s", key, e)
            return []
